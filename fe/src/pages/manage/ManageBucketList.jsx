...
export default function ManageBucketList() {
  ...
  return (
    <BucketList>
      ...
      {bucketFilter === "완료" && myBuckets.filter(bucket => bucket.complete).map(bucket => <ManageBucket bucket={bucket} key={bucket.seq} />)}
      {bucketFilter === "미완" && myBuckets.filter(bucket => !bucket.complete).map(bucket => <ManageBucket bucket={bucket} key={bucket.seq} />)}
    </BucketList>
  );
}
